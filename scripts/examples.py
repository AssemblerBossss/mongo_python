...
def setup_database():
    ...
    with MongoDBConnection(
        host="127.0.0.1", username="admin", password="admin", db_name="test"
    ) as conn:
...
def example_find_operations():
    ...
    with MongoDBConnection(
        ...
        cursor = conn.find("users", {"status": "active", "city": "Москва"})
        ...
        cursor = conn.find("users", {"age": {"$gte": 20, "$lte": 30}})
...
def example_projection():
    ...
    with MongoDBConnection(
        ...
        conn.print_results(
            cursor, limit=3, title="Активные пользователи (только имя и статус)"
        )
...
def example_count_documents():
    ...
    with MongoDBConnection(
        ...
        moscow_adults = conn.count("users", {"city": "Москва", "age": {"$gt": 25}})
...
def example_aggregation():
    ...
    with MongoDBConnection(
        ...
        cursor = conn.aggregation("users", pipeline)
        conn.print_results(
            cursor, title="Статистика по городам (активные пользователи)"
        )
...
def example_practical_scenarios():
    ...
    with MongoDBConnection(
        host="1127.0.0.1", username="admin", password="admin", db_name="test"
    ) as conn:
        ...
        query = {"skills": {"$all": required_skills}, "status": "active"}
