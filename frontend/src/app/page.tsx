...
import {Loader2, Layers, ChevronRight, Trash2, AlertTriangle, Plus, Upload, X} from "lucide-react";
    ...
    const scanImportMutation = useMutation({
        ...
        onSuccess: (json) => {
            ...
            for (const a of json.addresses) {
                byCollection.set(a.collection, (byCollection.get(a.collection) ?? 0) + a.imported);
            }
            const breakdown = Array.from(byCollection.entries())
                .map(([collection, count]) => `${count} -> ${collection}`)
                .join(", ");
            toast.success(`Imported ${json.total_imported} (${breakdown || "nothing"}), skipped ${json.total_skipped}.`);
            queryClient.invalidateQueries({queryKey: ["collections"]});
        },
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-slate-50">
            ...
            {collectionToDelete && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div
                        className="bg-[#001e2b] text-white w-full max-w-md rounded-lg shadow-2xl border border-gray-800 overflow-hidden">
                        <div className="p-6">
                            <div className="flex justify-between items-start mb-6">
                            ...
                            <div className="space-y-6">
                                <p className="text-gray-300">
                                    Are you sure you want to drop collection <span
                                    className="font-bold text-white">&quot;{collectionToDelete}&quot;</span>? This
                                ...
                                <div className="space-y-3">
                                    <Label htmlFor="confirmName" className="text-sm font-medium text-gray-300">
                                        Type <span
                                        className="font-bold text-white">&quot;{collectionToDelete}&quot;</span> to
                                        confirm
                                    </Label>
                                    <Input
                                        id="confirmName"
                                ...
                                {deleteError && (
                                    <div
                                        className="p-3 bg-red-900/20 border border-red-900/50 text-red-400 text-sm rounded">
                                        {deleteError}
                                    </div>
                                )}
